...
function WorldMap({ mapData, property, extrema, genders }) {
  ...
  useEffect(() => {
    ...
    if (mapData === null) {
      ...
      const propertyValuesPercentsNums = propertyValuesPercents.map((s) =>
        parseFloat(s)
      );
      const minPropPercent = min(propertyValuesPercentsNums);
      const maxPropPercent = max(propertyValuesPercentsNums);
      const color = genderColorsMap[property]
        ? genderColorsMap[property]
        : genderColorsMap["sumOtherGenders"];
      const colorScalePercent = scaleLinear()
      ...
      g.selectAll(".country")
        ...
        .attr("fill", (feature) =>
          colorScalePercent(feature.properties[propertyPercent])
        )
        ...
        .text((value) =>
          value.properties.text
            ? `${value.properties.name}: 
                    ...
                    ∑ Other Genders: ${
                      value.properties.sumOtherGendersPercent
                        ? value.properties.sumOtherGendersPercent
                        : 0
                    }%
                    `
      ...
      key.attr("style", "transform: translate(5%, 0) scale(0.9)");
